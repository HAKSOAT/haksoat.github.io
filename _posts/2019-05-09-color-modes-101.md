...
last_modified_at: 2019-05-09T01:20:02+01:00
header:
...
Image processing requires the knowledge of various concepts, and having an understanding of color modes is one of those concepts. The images you see on your screen consist of pixels (picture elements), zoomed out such that you do not get to see the pixels but a smooth representation of them.

Each pixel has a color, hence the need to understand color modes. Knowing about color modes gives you an insight into when you use which color mode and its advantage.

In this write-up, I'll explain three of the most used color modes for digital image signal processing tasks; this discussion may also be beneficial to artists involved in graphics design.
...
You'll get to learn a bit about each of them and their use cases, as I'll try to break them down in as few words as possible.
...
Grayscale images are 8-bit images, i.e., each pixel has a value made up of 8 bits. The value at 0 has the following binary digits (bits) 00000000, while the value at 255 has the following binary digits (bits) 11111111; in decimal values, this is a range of 0 to 255\.
...
RGB is made up of three channels (remember I talked about single channels in the Grayscale section), and each channel is made up of Red, Green, and Blue; from primary school integrated science or fundamental Physics, these are known to be primary colors.
...
Since CMYK uses the 0-255 color scale like Grayscale and RGB, each of the four channels which are 8 bits add up to be 32 bits.
...
You won't find a lot of digital images in the CMYK mode, except when about to be printed so you may not have to do a lot of conversion from CMYK to Grayscale.
...
Binary images are also known as the _bitmap_ color mode, and if you've worked with an image editing tool such as Adobe Photoshop before, you may have come across such before.
...
The object gets scanned (remember CMYK depending on the reflection of light) into a digital image and most likely gets converted into RGB mode which is the mode you'll find in digital displays.

It is important to note that CMYK doesn't automatically become RGB or otherwise; hence the appearance of an image changes if you attempt to use an image in CMYK mode for RGB or otherwise; proper conversion needs to be done.
...
*   Processing is faster on Grayscale images than you'll ever image on images with multiple channels.
